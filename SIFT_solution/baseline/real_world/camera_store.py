...
class ImageSubscriber:
    def __init__(self, topic_name):
        ...
        self.cv2_img = None
        self.time_stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")  # Create a time stamp for the current time
    ...
    def get_frame(self):
        self.cv2_img = numpy.array(list(reversed(self.cv2_img)))
        return self.cv2_img
        ...
        while True:
            ...
            if self.cv2_img is not None:
                ...
                for _ in range(2):
                    ...
                    cv2.imwrite(img_path, numpy.array(list(reversed(self.cv2_img))))
                    cv2.imshow("Processed Image", numpy.array(list(reversed(self.cv2_img))))
                    cv2.waitKey(1)
                    # cv2.destroyAllWindows()

                    if self.keyboard_value == 'None':
                        data = {
                            "image_id": img_count,
                            ...
                            "annotations": [
                                {
                                    "keyboard_value": 'None',
                                    "current_linear_velocity": 0.0,  # TODO: Get the current linear velocity from the robot
                                    "current_angular_velocity": 0.0  # TODO: Get the current angular velocity from the robot
                                }
                            ]
                        }
                    else:
